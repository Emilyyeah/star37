/* 美术素材库 — 素材列表页
   按游戏分区 + 按场景分类 + 搜索 + 网格卡片展示 */

import { useState, useMemo, useEffect } from 'react'

import {
  Search, Upload, Image, Download, Eye,
...
const SCENE_TABS = [
  ...
  { key: 'splash', label: '开屏' },
  { key: 'push', label: '推送图' },
  { key: 'social', label: '社媒配图' },
  { key: 'activity', label: '活动UI' },
]
...
const MOCK_ASSETS: AssetItem[] = [
  { id: 'ast-001', name: '618大促主KV', scene: 'banner', sceneName: 'Banner', gameProject: 'dl3d', gameName: '斗罗3D', width: 750, height: 400, fileSize: '328KB', creator: '设计师A', createdAt: '2026-06-01', tags: ['618', '大促', '夏日'], color: 'bg-blue-100' },
  { id: 'ast-002', name: '签到活动弹窗', scene: 'popup', sceneName: '弹窗', gameProject: 'dl3d', gameName: '斗罗3D', width: 600, height: 800, fileSize: '156KB', creator: '设计师B', createdAt: '2026-05-28', tags: ['签到', '弹窗'], color: 'bg-orange-100' },
  { id: 'ast-003', name: '新版本开屏图', scene: 'splash', sceneName: '开屏', gameProject: 'dlmmo', gameName: '斗罗MMO', width: 1080, height: 1920, fileSize: '512KB', creator: '设计师A', createdAt: '2026-05-25', tags: ['开屏', '新版本'], color: 'bg-purple-100' },
  { id: 'ast-004', name: '周年庆推送图', scene: 'push', sceneName: '推送图', gameProject: 'xxyg', gameName: '小小蚁国', width: 400, height: 300, fileSize: '89KB', creator: '设计师C', createdAt: '2026-05-20', tags: ['推送', '周年庆'], color: 'bg-green-100' },
  { id: 'ast-005', name: '抖音短视频封面', scene: 'social', sceneName: '社媒配图', gameProject: 'frxxt', gameName: '凡人修仙传', width: 1080, height: 1080, fileSize: '245KB', creator: '设计师B', createdAt: '2026-05-18', tags: ['抖音', '社媒'], color: 'bg-pink-100' },
  { id: 'ast-006', name: '端午活动Banner', scene: 'banner', sceneName: 'Banner', gameProject: 'dlmmo', gameName: '斗罗MMO', width: 750, height: 400, fileSize: '298KB', creator: '设计师A', createdAt: '2026-05-15', tags: ['端午', 'Banner'], color: 'bg-cyan-100' },
  { id: 'ast-007', name: '暑期活动UI主图', scene: 'activity', sceneName: '活动UI', gameProject: 'sc33', gameName: '生存33天', width: 750, height: 1334, fileSize: '478KB', creator: '设计师C', createdAt: '2026-06-02', tags: ['暑期', '活动'], color: 'bg-yellow-100' },
  { id: 'ast-008', name: '邀请好友分享图', scene: 'social', sceneName: '社媒配图', gameProject: 'zmsl', gameName: '织梦森林', width: 800, height: 600, fileSize: '167KB', creator: '设计师B', createdAt: '2026-05-10', tags: ['分享', '邀请'], color: 'bg-red-100' },
  { id: 'ast-009', name: '抽奖活动弹窗', scene: 'popup', sceneName: '弹窗', gameProject: 'dl3d', gameName: '斗罗3D', width: 600, height: 800, fileSize: '201KB', creator: '设计师A', createdAt: '2026-06-03', tags: ['抽奖', '弹窗'], color: 'bg-indigo-100' },
]
    ...
    <div className="group bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-md transition-all duration-200 hover:-translate-y-0.5">
      ...
      <div className="px-3 py-2.5">
        ...
        <div className="flex items-center justify-between mt-1.5">
          ...
          <span className="text-xs px-1.5 py-0.5 rounded bg-gray-100 text-gray-500 font-medium">{asset.sceneName}</span>
        </div>
...
export default function AssetLibraryPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return list
  }, [gameTab, sceneFilter, search])

  const totalPages = Math.max(1, Math.ceil(filtered.length / PAGE_SIZE))
  const paged = useMemo(() => filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE), [filtered, page])

  useEffect(() => { setPage(1) }, [gameTab, sceneFilter, search])
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="shrink-0 px-6 pt-5 pb-0">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full font-medium">{MOCK_ASSETS.length} 个素材</span>
          </div>
          <button
        ...
        <div className="flex items-center gap-1 border-b border-gray-200 -mx-6 px-6">
          {GAME_TABS.map((tab) => (
            <button key={tab.key} onClick={() => setGameTab(tab.key)}
      ...
      <div className="shrink-0 px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-1">
          {SCENE_TABS.map((tab) => (
            <button key={tab.key} onClick={() => setSceneFilter(tab.key)}
              className={cn('px-2.5 py-1 rounded text-xs font-medium transition-colors',
                sceneFilter === tab.key ? 'bg-gray-900 text-white' : 'text-gray-500 hover:bg-gray-100')}>
              {tab.label}
            </button>
          ))}
        </div>
        <div className="relative w-52">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="搜索素材名称或标签..."
